...
const ProjectCard = ({ project }) => {
    return (
        ...
        >
            ...
            >
                <img src={project.coverImage} alt="coverImage" className="mb-3" style={{ maxWidth: '100%', height: 'auto', borderRadius: '10px' }} />
                <h2 className="fs-5 text-center fw-bold mb-3">{project.name}</h2>
                <p className="fs-6 mb-3 description" style={{ lineHeight: "1.5", textAlign: "justify", maxHeight: '120px', overflow: 'hidden' }}>{project.description}</p>
                <div className="mb-3">
                    <p className="fw-bold text-center">Technologies Used</p>
                    <div className="d-flex flex-wrap">
                        {project.technologies.map((technology, index) => (
                            <span key={index} className="badge bg-secondary me-2 mb-2">{technology}</span>
                        ))}
                    </div>
                </div>
